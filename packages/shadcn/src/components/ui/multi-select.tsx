  ...
  return (
    <PopoverTrigger asChild>
      <Button
        ...
        className={cn(
          "flex h-auto min-h-9 w-fit items-center justify-between gap-2 overflow-hidden rounded-md border border-input bg-transparent px-3 py-1.5 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[placeholder]:text-primary dark:bg-muted/10  dark:hover:bg-muted/20 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-primary",
          className,
        )}
      >
        {children}
      ...
      {[...selectedValues]
        ...
        .map(value => (
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <XIcon className="size-2 text-primary group-hover:text-destructive" />
            )}
          </Badge>
        ))}
      <Badge
...
export function MultiSelectContent({
  ...
  return (
    <>
      ...
      <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-0 bg-[#141414] border border-[#2a2a2a] text-white">
...
export function MultiSelectItem({
  ...
  return (
    <CommandItem
      {...props}
      className={cn(
        "mb-1.5 flex min-h-9 cursor-pointer items-center gap-2 rounded-md border border-input bg-muted/10 px-3 py-1.5 text-sm text-primary shadow-xs transition-colors last:mb-0 aria-selected:border-ring aria-selected:bg-accent aria-selected:text-accent-foreground",
        props.className
      )}
      onSelect={() => {
        toggleValue(value)
