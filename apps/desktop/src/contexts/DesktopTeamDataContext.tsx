...
export function DesktopTeamDataProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTeams = useCallback(async () => {
    ...
    try {
      ...
      if (!shouldSkipCache) {
        console.log("[DesktopTeamData] Reading from SQLite cache");
  ...
  useEffect(() => {
    ...
    console.log("[DesktopTeamData] Registering realtime refresh callback");

    const unregister = registerRefreshCallback(() => {
      console.log("[DesktopTeamData] Realtime update - forcing fresh fetch");
  ...
  const refresh = useCallback(async () => {
    console.log("[DesktopTeamData] Manual refresh triggered");
    if (fetchTeamsRef.current) {
