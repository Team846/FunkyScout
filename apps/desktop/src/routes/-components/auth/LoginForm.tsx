...
export function LoginForm() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Let Supabase use its configured redirect URLs (from dashboard settings)
      // This works for localhost in dev and production domains in prod
      const success = await sendPasswordReset(resetEmail);
      if (success) {
        toast.success("Reset link sent! Check your email and open the link to reset your password.", {
          duration: 3000,
        });
